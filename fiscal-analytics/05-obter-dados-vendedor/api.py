...
URL = "https://apitotvsmoda.bhan.com.br/api/totvsmoda/analytics/v2/seller-fiscal-movement/search"
...
print("🚀 Iniciando consulta de Vendedores (Analytics + DEBUG)...")
...
while True:
    ...
    print(f"\n📄 Consultando página {page} de vendedores…")
    ...
    debug_file = f"debug_response_sellers_page_{page}.json"
    ...
    for item in items:
        all_sellers.append({
            "CodigoVendedor": item.get("code"),
            "NomeVendedor": item.get("name"),
        })
...
if df_sellers.empty:
    ...
    excel_file = f"vendedores_lista_{date_now}.xlsx"
    ...
    try:
        ...
        print(f"Total de registros exportados: {len(df_sellers)}")
