...
def fetch_all_invoices() -> List[Dict[str, Any]]:
    ...
    log("🚚 Iniciando busca de notas fiscais (somente shippingCompany)...")
...
# ==============================
# PROCESSA SOMENTE SHIPPING COMPANY
# ==============================

def process_shipping(nf: Dict[str, Any]) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    # Debug JSON
    debug_file = f"debug_shipping_{datetime.now():%Y%m%d_%H%M%S}.json"
    ...
    for nf in items:
        try:
            shippings.append(process_shipping(nf))
        except Exception as e:
            log(f"⚠️ Erro ao processar invoice {nf.get('invoiceCode')}: {e}")

    df_shipping = pd.DataFrame(shippings)

    # Exporta Excel
    excel_file = f"shipping_company_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
    df_shipping.to_excel(excel_file, index=False)

    log(f"✅ Excel gerado com sucesso: {excel_file}")
    log(f"📊 Total de registros exportados: {len(df_shipping)}")