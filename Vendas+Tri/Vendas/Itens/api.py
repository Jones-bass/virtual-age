...
# ==============================
# PROCESSA SOMENTE ITEMS
# ==============================

def process_items(nf: Dict[str, Any]) -> List[Dict[str, Any]]:
    processed = []
...
if __name__ == "__main__":
    ...
    # Debug JSON
    debug_file = f"debug_items_{datetime.now():%Y%m%d_%H%M%S}.json"
    ...
    for nf in items:
        try:
            all_items_processed.extend(process_items(nf))
        except Exception as e:
            log(f"⚠️ Erro ao processar invoice {nf.get('invoiceCode')}: {e}")

    df_items = pd.DataFrame(all_items_processed)

    # Exporta Excel
    excel_file = f"invoice_items_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
    df_items.to_excel(excel_file, index=False)

    log(f"✅ Excel gerado com sucesso: {excel_file}")
    log(f"📊 Total de itens exportados: {len(df_items)}")